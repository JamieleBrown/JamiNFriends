...
function onUpdate(elapsed)
    ...
    for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'mustPress') == false then
			setPropertyFromGroup('unspawnNotes', i, 'noAnimation', false); --Stops original animations
		end
	end
    ...
    if getProperty('dad.animation.curAnim.name') == 'attack2' and dadName == 'Fire' then
        ...
        makeAnimatedLuaSprite('eye', 'eye', 350, 150)
            addLuaSprite('eye', true)
            doTweenAlpha('eye', 'Wheel', 0, 1, 'linear')
        ShotEff = true
    else
        setProperty('eye.alpha', 0.1)
        ShotEff = false
    end
end
